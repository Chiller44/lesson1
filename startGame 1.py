...
def key_down(event):
    ...
    if key in mainframe.commands:
        ...
        if done:
            ...
            update_grid_cell()
            if game_state() == "win":
                grid_cell[GRID_LEN//2 - 1][GRID_LEN//2 - 1].configure(text="You", bg=BACKGROUND_COLOR_CELL_EMPTY)
                grid_cell[GRID_LEN//2 - 1][GRID_LEN//2].configure(text="Win", bg=BACKGROUND_COLOR_CELL_EMPTY)
            if game_state() == "lose":
                grid_cell[GRID_LEN // 2 - 1][GRID_LEN // 2 - 1].configure(text="You", bg=BACKGROUND_COLOR_CELL_EMPTY)
                grid_cell[GRID_LEN // 2 - 1][GRID_LEN // 2].configure(text="Lose", bg=BACKGROUND_COLOR_CELL_EMPTY)
